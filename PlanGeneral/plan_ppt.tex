...
\item Integrar Pensamiento Integral y  Alfabetizaci\'on (Tem\'aticas) 
\item Sociales: Ampliar mec\'anismos de Participaci\'on Ciudadana. 
\item Sociales: Incluir derecho-deber del voto. 
\item Habilidades de resoluci\'on de examenes.
\item Ojo con reacciones emocionales en Soc. 9no.
\item Espa\~nol: ?`Mito y Leyenda? 
\item Espa\~nol: Explicitar la aplicaci\'on del lenguajes
\end{enumerate}
...
\begin{frame}
\frametitle{Pasos de evaluaci\'on}
\begin{enumerate}
\item Establecer criterios.
\item Recoger informaci\'on.
\item Aplicar criterios a la informaci\'on.
\item Emitir recomendaciones, juicios y decisiones.
\end{enumerate}
\end{frame}
...
\ganttbar[progress=100,name=T]{Ejecuci\'on}{2014-11-01}{2014-12-20} \\
...
\begin{frame}
\frametitle{Fuentes de informaci\'on para la evaluaci\'on}
\begin{itemize}
\item \textbf{Experimentos}: Experimentaci\'on en Ciencias Sociales.
\item \textbf{Medidas}: Medidas del Comportamiento.
\item\textbf{An\'alisis sist\'emico}: Estudio de las interrelaciones entre variables
\item  \textbf{Interpretaciones}:  Filosofia hermen\'eutica y teorias de la interpretaci\'on, 
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Diversidad de la evaluaci\'on}
\begin{center}
\begin{tikzpicture}
 \node[rectangle,draw=black!50,fill=orange!25!green!50] (ev) at (0,0){Evaluaci\'on};
 
  \node[rectangle,draw=black!50,fill=orange!25!green!50] (ep) at (-4,1){Epistemolog\'ias};
 \node[rectangle,draw=black!50,fill=orange!25!green!50] (me) at (-2,2){M\'etodos};
 \node[rectangle,draw=black!50,fill=orange!25!green!50] (mp) at (0,1){Met\'aforas};
 \node[rectangle,draw=black!50,fill=orange!25!green!50] (ne) at (2,2) {Necesidades};
  \node[rectangle,draw=black!50,fill=orange!25!green!50] (cp) at (4,1){C. Pr\'acticas};
  
 \draw [->] (ep) to [out= 270, in=90](ev) ;
 \draw [->] (me) to [out= 270, in=90](ev) ;
 \draw [->] (mp) to [out= 270, in=90](ev) ;
 \draw [->] (ne) to [out= 270, in=90](ev) ;
 \draw [->] (cp) to [out= 270, in=90](ev) ;
...
\begin{frame}
\frametitle{Epistemolog\'ias}
\begin{center}
\begin{tikzpicture}
 \node[rectangle,draw=black!50,fill=orange!25!green!50] (obj) at (-1.5,3 ){Objectivism};
  \node[rectangle,draw=black!50,fill=orange!25!green!50] (subj) at (1.5,3 ){Subjectivism};
 \node[rectangle,draw=black!50,fill=orange!25!green!50] (oPro)  at (-4,4 ){Datos. Externos};
  \node[rectangle,draw=black!50,fill=orange!25!green!50] (sPro) at (4,4 ){Experiencia. Interna};  
   \node[rectangle,draw=black!50,fill=orange!25!green!50] (oCon)  at (-4,2 ){Complej. Interact. };
  \node[rectangle,draw=black!50,fill=orange!25!green!50] (sCon) at (4,2 ){Contrad. y Fiabilidad.};  
  
 \draw [->] (obj) to [out= 180, in=270](oPro) ;
 \draw [->] (subj) to [out= 0, in=270] (sPro) ;
  \draw [->] (obj) to [out= 180, in=90](oCon) ;
 \draw [->] (subj) to [out= 0, in=90] (sCon) ;
 
\node[rectangle,draw=black!50,fill=blue!25!green!50] (ut)  at (-1.5,0 ){Utilitarian};
\node[rectangle,draw=black!50,fill=blue!25!green!50] (in) at (1.5,0 ){Intuitionist-Pluralist};  
\draw[->>]  (obj) to (ut);
\draw [->>] (subj) to (in) ;

\node[rectangle,draw=black!50,fill=blue!25!green!50] (impa)  at (-4,1 ){Imparcial};
\node[rectangle,draw=black!50,fill=blue!25!green!50] (intu) at (4,1){Intuitivo};  
\node[rectangle,draw=black!50,fill=blue!25!green!50] (overall)  at (-4,-1 ){Del conjunto};
\node[rectangle,draw=black!50,fill=blue!25!green!50] (ind) at (4,-1){Individuo};  

 \draw [->] (ut) to [out= 180, in=270](impa) ;
 \draw [->] (in) to [out= 0, in=270] (intu) ;
 \draw [->] (ut) to [out= 180, in=90] (overall) ;
 \draw [->] (in) to [out= 0, in=90] (ind);
...
\item Informaci\'on Cualitativa
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Me\'taforas}
Figura en la que el significado de un t\'ermino o frase se transfiere de un objeto, al cual se le asigna usualmente, a otro para proveer una nueva perspectiva. \\
Chismes $\leftrightarrow$ Pandemias
\end{frame}
