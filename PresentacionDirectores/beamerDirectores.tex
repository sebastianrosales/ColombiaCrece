...
\item Relaci\'on con su entorno.
\item Reconocimiento de la pluralidad.  
\end{enumerate}
\end{enumerate}
\end{frame}
...
\begin{center}
\begin{tikzpicture}
\node (pronouns) at (0,3) {Pronombres};
\node (tb) at (2.6,-1.5) {Determinantes, Art\'iculos, Singular y plural};

\node(th) at ( -2.6,-1.5 ) {To Be};
\node (art) at (0,-3){ Adjetivos}; 
\node (sing) at (-2.6,1.5){Preguntas WH}; 
\node (tenses) at (2.6,1.5){Tiempos presente, pasado y futuro}; 
\draw[->]  (pronouns) to (sing);
\draw[->]  (sing) to (th);
\draw[->]  (th) to (art);
\draw[->]  (art) to (tb);
\draw[->]  (tb) to (tenses);
\draw[->]  (tenses) to (pronouns);
\end{tikzpicture}
\end{center}
\end{frame}
\section{2015}
